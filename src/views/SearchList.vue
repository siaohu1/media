  ...
  <section>
    <div class="stimulation_container">
      ...
      <div class="stimulation_container_center">
        ...
        <p>Search Results for: {{wordText}}</p>
        <div v-show="!result">NOT FIND </div>
        <div class="center_container" v-if="result" style="margin-top: 30px">
          <ul>
          ...
          <div class="pagination" v-show="result">
            <el-pagination
              background
              layout="prev, pager, next"
              :page-size="pageSize"
              :total="count"
              @current-change="getInsiht"
...
<script>
  ...
  export default {
    ...
    mounted(){
      this.wordText = this.$route.query.word
      this.getInsiht()
    },
    methods: {
      getInsiht(page =1,title = this.$route.query.word){
        apiRequest.Search({
